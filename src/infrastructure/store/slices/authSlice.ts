...
export const login = createAsyncThunk(
  'auth/login',
  async ({ correo, contrasena }: { correo: string; contrasena: string }, { rejectWithValue }) => {
    try {
...
export const checkAuthStatus = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Sesión no iniciada');
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(login.fulfilled, (state, action) => {
        ...
        // Si el usuario tiene una sede asignada (no es admin o es admin con sede default)

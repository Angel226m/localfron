...
// Función para extraer el array de idiomas de la respuesta
const extractIdiomasArray = (response: any): Idioma[] => {
  ...
  if (response && typeof response === 'object') {
    ...
    // Si no encuentra ninguna propiedad conocida, ver si el objeto tiene propiedades que parezcan idiomas
    const values = Object.values(response);
    const arrayValue = values.find(value => Array.isArray(value));
    if (arrayValue) {
      return arrayValue as Idioma[];
    }
  }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchIdiomas.fulfilled, (state, action: PayloadAction<Idioma[]>) => {
        console.log('Idiomas recibidos en slice:', action.payload);
        console.log('Es array?:', Array.isArray(action.payload));
        
        // Asegurar que siempre sea un array
        state.idiomas = Array.isArray(action.payload) ? action.payload : [];
        state.loading = false;
      })
      .addCase(fetchIdiomas.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string || 'Error desconocido';
        state.idiomas = []; // Asegurar que sea un array en caso de error
      })
      ...
      .addCase(createIdioma.fulfilled, (state, action: PayloadAction<Idioma>) => {
        // Asegurar que idiomas sea un array antes de hacer push
        if (Array.isArray(state.idiomas)) {
          state.idiomas.push(action.payload);
        } else {
      ...
      .addCase(updateIdioma.fulfilled, (state, action: PayloadAction<Idioma>) => {
        // Asegurar que idiomas sea un array antes de buscar
        if (Array.isArray(state.idiomas)) {
      ...
      .addCase(deleteIdioma.fulfilled, (state, action: PayloadAction<number>) => {
        // Asegurar que idiomas sea un array antes de filtrar
        if (Array.isArray(state.idiomas)) {
